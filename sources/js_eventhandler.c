...
static JSValue js_eventhandler_send_event(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    if(argc > 1) {
        if(JS_IsString(argv[1])) {
            ...
            js_session_t *jss = JS_GetOpaque(argv[1], js_session_get_classid(ctx));
            if(!jss || !jss->session) {
...
static void js_eventhandler_finalizer(JSRuntime *rt, JSValue val) {
    js_eventhandler_t *js_eventhandler = JS_GetOpaque(val, js_eventhandler_get_classid2(rt));
...
JSClassID js_eventhandler_get_classid2(JSRuntime *rt) {
    ...
    return script->class_id_eventhandler;
}
JSClassID js_eventhandler_get_classid(JSContext *ctx) {
    return  js_eventhandler_get_classid2(JS_GetRuntime(ctx));
}
...
switch_status_t js_eventhandler_class_register(JSContext *ctx, JSValue global_obj, JSClassID class_id) {
    ...
    script->class_id_eventhandler = class_id;
    ...
    JS_SetPropertyFunctionList(ctx, obj_proto, js_eventhandler_proto_funcs, QJS_ARRAY_SIZE(js_eventhandler_proto_funcs));
