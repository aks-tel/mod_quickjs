...
static void js_event_finalizer(JSRuntime *rt, JSValue val) {
    js_event_t *js_event = JS_GetOpaque(val, js_event_get_classid2(rt));
...
JSClassID js_event_get_classid2(JSRuntime *rt) {
    ...
    return script->class_id_event;
}
JSClassID js_event_get_classid(JSContext *ctx) {
    return  js_event_get_classid2(JS_GetRuntime(ctx));
}
...
switch_status_t js_event_class_register(JSContext *ctx, JSValue global_obj, JSClassID class_id) {
    ...
    script->class_id_event = class_id;
    ...
    JS_SetPropertyFunctionList(ctx, obj_proto, js_event_proto_funcs, QJS_ARRAY_SIZE(js_event_proto_funcs));
...
JSValue js_event_object_create(JSContext *ctx, switch_event_t *event) {
    ...
    JS_SetPropertyFunctionList(ctx, proto, js_event_proto_funcs, QJS_ARRAY_SIZE(js_event_proto_funcs));
